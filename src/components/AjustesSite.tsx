...
function SortableAssuntoRow({
  id,
  value,
  placeholder,
  onChange,
  onDelete,
}: SortableAssuntoRowProps) {
...
export function AjustesSite({ settings, loading = false, onSave }: AjustesSiteProps) {
  ...
  const paColumnTabs = useMemo(
    () => form.columnTabs.filter((aba) => aba.scope === "PA").sort((a, b) => a.order - b.order),
    [form.columnTabs],
  );

  const duColumnTabs = useMemo(
    () => form.columnTabs.filter((aba) => aba.scope === "DU").sort((a, b) => a.order - b.order),
    [form.columnTabs],
  );
  ...
  const selectedFlowAction = useMemo(() => {
    ...
    if (selectedFlowActionIndex < 0 || selectedFlowActionIndex >= form.paFlowActions.length)
      return null;
                  ...
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updatePAFlowAction(index, { fromState: e.target.value as PAFlowState })
                        }
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      >
                        {PA_FLOW_STATES.map((estado) => (
                          <option key={`from-${estado}`} value={estado}>
                            {estado}
                          </option>
                        ))}
                      </select>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        value={acao.toState}
                        onChange={(e) =>
                          updatePAFlowAction(index, { toState: e.target.value as PAFlowState })
                        }
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      >
                        {PA_FLOW_STATES.map((estado) => (
                          <option key={`to-${estado}`} value={estado}>
                            {estado}
                          </option>
                        ))}
                      </select>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        value={acao.role}
                        onChange={(e) =>
                          updatePAFlowAction(index, { role: e.target.value as PAFlowRole })
                        }
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        value={acao.track}
                        onChange={(e) =>
                          updatePAFlowAction(index, { track: e.target.value as PAFlowTrack })
                        }
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      >
                        {PA_FLOW_TRACK_OPTIONS.map((opt) => (
                          <option key={`track-${opt.value}`} value={opt.value}>
                            {opt.label}
                          </option>
                        ))}
                      </select>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        value={acao.order}
                        onChange={(e) =>
                          updatePAFlowAction(index, { order: Number(e.target.value) || 0 })
                        }
                      />
                    </div>
                    ...
                    <div className="flex items-end justify-between gap-2">
                      ...
                      <Button
                        type="button"
                        variant="outline"
                        onClick={() => removePAFlowAction(index)}
                      >
                        <Trash2 className="mr-2 h-4 w-4" />
                        Excluir
                      </Button>
                  ...
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                    <div className="space-y-1.5">
                      ...
                      <select
                        value={acao.fromState}
                        onChange={(e) =>
                          updatePAFlowAction(index, {
                            fromState: e.target.value as PAFlowState,
                            track: trackPadrao,
                          })
                        }
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      >
                        {PA_FLOW_STATES.map((estado) => (
                          <option key={`from-simple-${estado}`} value={estado}>
                            {PA_FLOW_STATE_LABELS[estado]}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updatePAFlowAction(index, {
                            toState: e.target.value as PAFlowState,
                            track: trackPadrao,
                          })
                        }
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      >
                        {PA_FLOW_STATES.map((estado) => (
                          <option key={`to-simple-${estado}`} value={estado}>
                            {PA_FLOW_STATE_LABELS[estado]}
                          </option>
                        ))}
                      </select>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updatePAFlowAction(index, {
                            role: e.target.value as PAFlowRole,
                            track: trackPadrao,
                          })
                        }
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      >
                        {PA_FLOW_ROLE_OPTIONS.map((opt) => (
                          <option key={`role-simple-${opt.value}`} value={opt.value}>
                            {opt.label}
                          </option>
                        ))}
                      </select>
  ...
  const handleSave = async () => {
    ...
    if (
      processualFields.some((campo) => !Number.isFinite(form[campo]) || Number(form[campo]) <= 0)
    ) {
    ...
    const duBoardColumns = normalizarDUBoardColumns(form.duBoardColumns, DEFAULT_DU_BOARD_COLUMNS);

    const columnTabs = normalizarColumnTabs(form.columnTabs, DEFAULT_COLUMN_TABS);
          ...
          <Collapsible open={openPA} onOpenChange={setOpenPA}>
            <div className="rounded-2xl border border-border bg-background/40 p-4">
              <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                ...
                <ChevronDown
                  className={`h-4 w-4 text-muted-foreground transition-transform ${openPA ? "rotate-180" : ""}`}
                />
              </CollapsibleTrigger>
                  ...
                  <section className="space-y-4">
                    <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          Lista usada no dropdown de assunto quando o tipo de procedimento for
                          Sindicância.
                        </p>
                      </div>
                      <ChevronDown
                        className={`h-4 w-4 text-muted-foreground transition-transform ${openPAAssuntos ? "rotate-180" : ""}`}
                      />
                    </CollapsibleTrigger>
                    ...
                    <CollapsibleContent className="pt-2">
                      <div className="space-y-3">
                        ...
                        >
                          ...
                          <DragOverlay>
                            {dragAssuntoPALabel ? (
                              <AssuntoDragPreview label={dragAssuntoPALabel} />
                            ) : null}
                          </DragOverlay>
                        </DndContext>
